      ...
      <div className="bg-white border-b border-[#e5e5ea]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 sm:py-4 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[#6e6e73] text-[11px] sm:text-[12px] hidden sm:block">{podcast.uploadedPlatforms.map(u => u.platform).join(', ')}</p>
          </div>
          <button
            onClick={() => router.push('/compare')}
            className="hidden sm:block text-[13px] bg-[#f5f5f7] hover:bg-[#e5e5ea] text-[#6e6e73] px-4 py-2 rounded-xl transition-colors font-medium border border-[#e5e5ea] flex-shrink-0"
          >
            Сравнить
          </button>
          <button
            onClick={() => router.push(`/${podcastId}/setup`)}
            className="text-[13px] bg-[#f5f5f7] hover:bg-[#e5e5ea] text-[#1d1d1f] px-3 sm:px-4 py-2 rounded-xl transition-colors font-medium border border-[#e5e5ea] flex-shrink-0"
          >
            + Данные
          </button>
        </div>
